...
- **Env vars** are set in the Vercel project (production) — DATABASE_URL, JWT_SECRET, OAUTH_SERVER_URL, OWNER_OPEN_ID, GEMINI_API_KEY, LIVESCORE_API_KEY, ADMIN_NOTIFY_EMAIL, VITE_*. Add `FLUTTERWAVE_*` (or `PAYSTACK_*`) / `RESEND_API_KEY` there to go live on payments/email. Manage via `vercel env`.
